const PreviousResult = require('../../model/PreviousResults/MorningResult')
...
const getPreviousResult =  async(req, res) => {
 ...
 result.morningteerresult= await PreviousResult.find({}).limit(limit).skip(startIndex).sort([['date', -1]]).exec()
 res.render("PreviewsResult/MorningTeerResult/showPreviousResult", {
    result,
    docTitle:`Morning Previous Result`
   });
...
const getPreviousSearchTeerResult =  async(req, res) => {
    try{
     if(!req.body.startdate && !req.body.enddate){
         ...
         res.redirect("/morningteerresult")
     }
     let result={}
      
   result.morningteerresult=await  PreviousResult.find({
         date: {
...
const addPreviousResult = async (req, res) => {
    ...
    try {
        res.render("PreviewsResult/MorningTeerResult/addPreviousResult",{docTitle:`Add Previous Result`})
    } catch (error) {
        req.flash(
            'error_msg',
            error.message
...
const deletePreviousResult = async (req, res) => {
    ...
    try {
        ...
        foundpreviousresult.deleteOne()
         req.flash(
             'success_msg',
             'Previous Result delete successfully'
         );
        res.redirect("/morningteerresult")
    } catch (error) {
        req.flash(
